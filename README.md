...
## `threemf modify IN OUT --code CODE`

Run Python code over a mesh's vertex (`V`, nx3 float) and triangle (`T`, mx3 int) arrays, then save the result as a new 3MF. In scope: `V`, `T`, `np`, `trimesh`, `mesh` (lib3mf mesh object), `model` (lib3mf model). Leave the modified arrays under the names `V` and `T`. Other meshes and attachments are preserved.

```console
$ threemf modify in.3mf out.3mf --code 'V[:, 2] += 5  # lift 5mm'
```

Pipe code from a file with `--code -`:

```console
$ cat fix.py | threemf modify in.3mf out.3mf --code -
```
